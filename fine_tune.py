...
def add_token_positions(encodings, answers, tokenizer):
    ...
    for idx in range(len(answers)):
        start_positions.append(
            encodings.char_to_token(idx, answers[idx]["answer_start"])
        )
        if answers[idx]["answer_end"] is None:
            end_positions.append(
                encodings.char_to_token(idx, answers[idx]["answer_end"])
            )
        else:
            end_positions.append(
                encodings.char_to_token(idx, answers[idx]["answer_end"] - 1)
            )
...
def checkpoint_model(
    model, model_path, current_score, epoch, history, patience, mode="max"
):
...
def train_and_save_model(
    ...
    for epoch in range(epochs):
        ...
        if checkpoint_model(
            model, model_path, loss.item(), epoch, history, patience, mode="min"
        ):
            break
